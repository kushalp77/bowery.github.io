...
function setCountry(oCountrySel,oCity_StateSel)
{
    var countriesNew = new Array();
    for (region in countries)
    {
        var countryArr;
        if (countries[region])
        {
            ...
            for (var i = 0; i < countryArr.length; i++)
            {
                ...
                {
                    countriesNew.push(countryArr[i]);
                }
            }
        }
    }
    countriesNew.sort();
    var oCountrySel = document.getElementById("country");
    oCountrySel.disabled = false;
    var oCity_StateSel = document.getElementById("state");
    oCity_StateSel.disabled = true;
    oCountrySel.options[0] = new Option('SELECT COUNTRY','');
    countryArr = countries[region].split('|');
    for (var i = 0; i < countriesNew.length; i++) 
    {
        oCountrySel.options[i + 1] = new Option(countriesNew[i], countriesNew[i]);
    }
}
function setState(oCountrySel,oCity_StateSel)
{    
    var oCountrySel = document.getElementById("country");        
    var oCity_StateSel = document.getElementById("state");         
    oCity_StateSel.length = 0;
    var country = oCountrySel.value;    
